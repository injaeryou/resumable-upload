...
| `store_url` | bool | `False` | Persist upload URLs for cross-session resume, including the partial URLs of a `parallel_uploads=N` upload; a stored URL the server answers `404`/`410` for is dropped and recreated |
