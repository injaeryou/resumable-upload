...
| `disable_termination` | bool | `False` | Reject client DELETEs with `405` and drop `termination` from `Tus-Extension`; `terminate_upload()` still works |
| `disable_concatenation` | bool | `False` | Reject any `Upload-Concat` request with `400` and drop both concatenation extensions |
| `behind_proxy` | bool | `False` | Build absolute `Location` URLs from `X-Forwarded-Proto`/`X-Forwarded-Host` (falling back to `Host`, then to a relative path). Relative stays the default — it is proxy-safe by construction. |
| `location_base_url` | str | `None` | Fixed absolute prefix for `Location` URLs (e.g. a CDN origin). Takes precedence over `behind_proxy`. |
...
The handler speaks HTTP/1.1 with keep-alive when hosted by a threading server (`http.server.ThreadingHTTPServer`, which is what `resumable-upload serve` uses). On a plain single-threaded `HTTPServer` like the example above it falls back to HTTP/1.0 and closes after each response, because a parked keep-alive socket would block every other client for `request_timeout`. Every response is framed with `Content-Length`, `HEAD` responses never carry a body, and a request whose body the handler does not read is answered with `Connection: close`.
