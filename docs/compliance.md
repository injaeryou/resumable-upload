...
| **Networking / TLS** | ✅ UNIX socket · ✅ HTTP/2 + h2c · ✅ TLS 1.2/1.3 modes · ✅ network + request-completion timeouts | ✅ stdlib `http.server` (CLI, thread-per-connection) / any ASGI server · ✅ Slowloris socket timeout (`--request-timeout`) · ❌ no worker cap — put the bundled server behind a reverse proxy · ❌ built-in TLS (terminate at your reverse proxy or ASGI server) |
...
| **Graceful shutdown** | ✅ SIGINT/SIGTERM drain with `-shutdown-timeout` | ✅ SIGINT/SIGTERM stop accepting and join in-flight request threads · ❌ no dedicated shutdown-timeout knob (the drain is bounded by `--request-timeout`, which reaps stalled connections) |
...
| **Retry** | ✅ `retryDelays` array `[0,1s,3s,5s]` · ✅ `onShouldRetry` override | ✅ `max_retries` + exponential backoff (cap 60s) · ✅ same default policy plus `408`/`429` (no retry on other 4xx; `409` re-syncs the offset) · ✅ `on_should_retry` override · ✅ `stop_event` interrupts waits |
