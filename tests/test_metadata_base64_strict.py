"""Regression: Upload-Metadata base64 must be validated, not silently
truncated (b64decode drops non-alphabet characters without validate=True).
"""
...
class TestMetadataBase64Validation:
    def test_non_alphabet_base64_rejected(self, storage):
        ...
        status, _, body = server.handle_request(
            "POST",
            "/files",
            _h(**{"Upload-Length": "1", "Upload-Metadata": "filename ####"}),
            b"",
        )
        assert status == 400
