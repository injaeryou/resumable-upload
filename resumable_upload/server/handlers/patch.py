...
async def handle_patch_async(
    ...
    Identical logic but every storage I/O call goes through the
    ``Storage._async`` surface so true-async backends never block the loop.
    """
    ...
    if upload["upload_length"] is None:
        ...
        if server.max_size > 0 and committed_length > server.max_size:
            return server._error_response(413, "Upload exceeds maximum size")
        await server.storage.set_upload_length_async(upload_id, committed_length)
        upload["upload_length"] = committed_length
    elif patch_upload_length is not None:
        try:
            resent_length = int(patch_upload_length)
    ...
    if server.max_chunk_size > 0 and len(body) > server.max_chunk_size:
        logger.warning("Chunk size %s exceeds max_chunk_size %s", len(body), server.max_chunk_size)
        return server._error_response(413, "Chunk exceeds maximum chunk size")

    await server.storage.write_chunk_async(upload_id, upload_offset, body)
    if not await server.storage.update_offset_atomic_async(upload_id, upload_offset, new_offset):
        return server._error_response(409, "Concurrent write conflict; use HEAD to re-sync")
    ...
    if new_offset >= upload["upload_length"] and await server.storage.complete_upload_async(
        upload_id
    ):
        if server._metrics is not None:
            server._metrics.inc("tusd_uploads_finished_total")
